...
export interface ForgetPasswordFieldResponse {
    email: ErrorResponse;
    ok: boolean;
}
export const validateForgetPasswordForm = async (prev: any, formData: FormData): Promise<ForgetPasswordFieldResponse> => {
    const email = formData.get('email')?.toString() || "";

    const result: ForgetPasswordFieldResponse = {
        email: {
            error: false,
            value: email
        },
...
export interface PasswordFieldResponse {
